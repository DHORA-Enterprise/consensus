...
class Block:
    def __init__(self, index, timestamp, transactions, staker, prev_hash):
        self.index = index
...
class Blockchain:
    ...
    def add_block(self, transactions, staker):
        prev_block = self.chain[-1]
        new_block = Block(len(self.chain), time.time(), transactions, staker, prev_block.hash)
    ...
    def is_block_valid(self, new_block, prev_block):
        ...
        if new_block.calculate_hash() != new_block.hash:
            return False
        if new_block.staker not in self.stakers:
            return False
        return True

    def stake_validation(self):
